...
- [x] **2026-06-20 — Phase 03 gap-fixes: Inc 1–5 (BaaS auth hardening).** **Inc 1** (commit `406f4a0`): platform auth correctness — `auth-session.service` now carries `sessionId` in access JWT, `auth-token.service` rotates refresh on use (the half-broken refresh/refresh-token join is fixed), `jwt.strategy` reads `JWT_SECRET_FILE` like realtime does, formal `@CurrentUser` decorator + 3-arg `extractRequestContext(req)` (IP from `x-forwarded-for`, UA) for audit emits, deleted dead `auth.service.ts` duplicate. **Inc 2** (commit `9d060f0`): Redis-backed `AuthRateLimiter` (Lua atomic incr+expire) shared by platform auth + app-auth; failed login → 429 after limit; 429 with `Retry-After`. **Inc 3** (commit `10b4856`): platform-user TOTP MFA — `MfaService` (otplib v13 `generateSync`/`verifySync`/`keyuri`), `POST /auth/mfa/setup` (secret + QR URI), `/auth/mfa/verify` (confirm → enable), `/auth/mfa/challenge` (TOTP-on-login), `mfaEnabled`/`mfaSecret` columns already on `User` (now encrypted at rest via CryptoService). **Inc 4** (commit `2ea4e76`): BaaS **magic-code (OTP) sign-in** — first non-password BaaS auth method per project, Redis-backed bcrypt-hashed codes, 10-min TTL, attempt lock, 3-sends/5-verifies per window, low-case email normalization, Stalwart delivery via `SmtpSendService` (omits `from` → platform `SMTP_FROM`). Routes `POST projects/:projectId/auth/magic-code` and `/verify-magic-code`. `requestCode` returns 200 for unknown emails (no enumeration); user is created on first successful verify. Cross-project OTP isolation verified. Events `auth.magic_code_sent` / `auth.magic_code_verified` carry actor/IP/UA. **Inc 5** (commit `e1bfec6`): per-project **OAuth (Google + GitHub)** via manual Auth-Code flow with per-project encrypted creds (CryptoService AES-256-GCM), `AuthProvider` table, provider CRUD (`projects/:id/auth/providers/:provider`, admin-gated), `MockOAuthProvider` for testing without real creds. **App-user JWT** — all BaaS auth flows (login, magic-code, OAuth) now issue signed JWTs (`{scope:'app', projectId, sub, sessionId, type:'access'}` 15m / `type:'refresh'` 7d); `AppAuthTokenService` with rotation on use; `AppJwtStrategy` + `AppJwtGuard` + `@CurrentAppUser`; `AppSession.tokenHash` now stores bcrypt(refreshToken); `revokedAt`/`lastUsedAt` added to `projects.app_sessions`. Events `auth.token_refreshed`, `auth.refresh_rejected`, `auth.oauth_authorize_started`, `auth.oauth_signin_succeeded`, `auth.oauth_signin_failed`, `auth.provider_removed`. Verified: mock OAuth start→callback→AppUser created→JWT pair; `/me` with app JWT; token refresh rotation (old rejected after rotation); password login issues JWT; provider config encrypted in DB, masked on read.
...
*Last updated: 2026-06-20 (Phase 03 gap-fixes Inc 1–5 verified; OAuth + app-user JWT live on VPS)*
