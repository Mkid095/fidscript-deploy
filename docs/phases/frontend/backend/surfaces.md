...
MKT-01 GET ?{type,category,search,sort,limit,offset} · 02 GET `/featured` · 03 GET `/categories` · 04 GET `:slug` · 05 POST `:slug/reviews` {rating,title?,content?} · 06 POST `:slug/download` · 07 POST `/submit` {type,name,...} · 08 GET `/my/submissions` · 09 PATCH `/items/:id` ⚠ no owner check · 10 POST `/items/:id/approve` (admin) · 11 `/reject` · 12 `/featured` {featured} · 13 `/verify`.
Events: `marketplace.item.submitted`, `.approved`, `marketplace.review.created`.

## Registry & Health (public)
SVC-01 GET `/api/v1/services` · 02 GET `/api/v1/services/:name` · 03 GET `/api/v1/health` (aggregate) · 04 GET `/api/v1/health/live` · 05 GET `/api/v1/health/ready`.
