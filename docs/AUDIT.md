...
| **Email (Stalwart)** | PARTIAL | `verifyDomain` now queries real Cloudflare DNS records (DKIM TXT, SPF, DMARC, MX) via `MailDnsService` — no more hardcoded booleans. DKIM Ed25519 key generation via `MailDnsService.generateDkimKey`; private key stored on Stalwart volume; public key published as TXT DNS record. MX/SPF/DMARC created via `DnsProvider` interface (Phase 07 Cloudflare wiring). Stalwart ports exposed in docker-compose (25/465/587/993/8443). Still stub: actual SMTP send via Stalwart (nodemailer SMTP still used), mailbox creation not mirrored to Stalwart API, inbound delivery not wired. |
