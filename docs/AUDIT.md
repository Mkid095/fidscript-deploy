...
| **Email (Stalwart)** | PARTIAL | Schema restructured (email.domains/mailboxes/aliases/sender_identities/api_keys/messages/catch_all_rules). StalwartJmapService wraps all JMAP admin calls (x:Domain/set, x:Account/set, jmapIdentityCreate, jmapSieveScriptSet). Mailboxes and domains are created in Stalwart via JMAP. DKIM Ed25519 keys generated; private key on Stalwart volume, public key as TXT DNS record. MX/SPF/DMARC via DnsProvider. Message bodies stored in MinIO (not Postgres). Inbound webhook receives from Stalwart sieve notify, stores to MinIO, emits email.received. All events wired (sent, received, delivered, bounced, mailbox/alias/identity/api_key CRUD). Still: SMTP relay fallback not wired, delivery status tracking incomplete. |
...
| **Realtime** | PARTIAL | Socket.IO gateway is wired (socket.io deps present). JWT auth reads `JWT_SECRET_FILE` (not hardcoded). `validateChannelToken` actually bcrypt-compares the stored hash. Presence persisted to Redis (TTL). Private channels auto-generate bcrypt-hashed access tokens at creation. Platform events still do not fan out to clients — the EventService bridge to `server.emit` is not yet implemented. |
