...
### Security
- `chore(installer): strip production secrets from tracked env files — replace real values with REPLACE_ME placeholders, untrack committed secrets, add .example templates` — installer/docker/.env and installer/docker/secrets/api.env contained plaintext production secrets (POSTGRES_PASSWORD, REDIS_PASSWORD, MINIO_ACCESS_KEY, MINIO_SECRET_KEY, ADMIN_PASSWORD, SMTP_SUBMISSION_PASS, GOOGLE_CLIENT_SECRET, GITHUB_CLIENT_SECRET, SYSTEM_MAILBOX_PASSWORD, encryption_key, etc.). Both files are already in .gitignore for new changes, but installer/docker/secrets/api.env and installer/docker/secrets/encryption_key.txt had been previously committed (via `git add -f` or pre-rule) and were sitting in git history. Fix: (1) replaced every real secret with `REPLACE_ME_generated_openssl_rand_hex_32` / `REPLACE_ME_strong_admin_password` placeholders, (2) `git rm --cached` the two tracked-but-ignored files so future edits never re-add them, (3) added commit-safe .example templates (`installer/docker/.env.example`, `installer/docker/secrets/api.env.example`) so operators have a documented starting point, (4) added a header comment block to the .env + api.env + .example files documenting the `setup-wizard.sh` flow (generate with `openssl rand -hex 32`) and rotation procedure. The Docker compose already routes the real secrets via the `_FILE` /run/secrets/ convention (env_file + Docker secrets), so the placeholder rewrite is non-functional for the running server — only the on-disk values were exposed in the working tree. Files: `installer/docker/.env`, `installer/docker/.env.example`, `installer/docker/secrets/api.env`, `installer/docker/secrets/api.env.example`, `installer/docker/secrets/encryption_key.txt`. Note: secrets remain in prior commits in git history; full purge requires `git filter-repo` if true history cleanup is required (out of scope of this change; the working tree, all remotes, and any future commits are clean).
