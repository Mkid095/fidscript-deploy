...
export type AntiBanQuality = 'GREEN' | 'YELLOW' | 'RED';

export type AntiBan = {
  ENABLED: boolean;
  RATE_LIMIT_CONTACT_MS: number;
  RATE_LIMIT_CONTACT_HOURLY: number;
  RATE_LIMIT_BURST: number;
  RATE_LIMIT_GLOBAL_THROUGHPUT: number;
  BLOCK_THRESHOLD: number;
  SUPPRESSION_TTL_DAYS: number;
  QUALITY_ALERT_THRESHOLD: AntiBanQuality;
  AUTO_PAUSE_ON_RED_DAYS: number;
};
...
export class ConfigService {
  ...
  private envProcess(): Env {
    return {
      SERVER: {
        NAME: process.env?.SERVER_NAME || 'fidscript',
      ...
      ANTI_BAN: {
        ...
        RATE_LIMIT_CONTACT_MS: Number.parseInt(process.env?.ANTIBAN_RATE_LIMIT_CONTACT_MS) || 6000,
        RATE_LIMIT_CONTACT_HOURLY: Number.parseInt(process.env?.ANTIBAN_RATE_LIMIT_CONTACT_HOURLY) || 600,
        RATE_LIMIT_BURST: Number.parseInt(process.env?.ANTIBAN_RATE_LIMIT_BURST) || 45,
        RATE_LIMIT_GLOBAL_THROUGHPUT: Number.parseInt(process.env?.ANTIBAN_RATE_LIMIT_GLOBAL_THROUGHPUT) || 100,
        BLOCK_THRESHOLD: Number.parseInt(process.env?.ANTIBAN_BLOCK_THRESHOLD) || 3,
        SUPPRESSION_TTL_DAYS: Number.parseInt(process.env?.ANTIBAN_SUPPRESSION_TTL_DAYS) || 30,
        QUALITY_ALERT_THRESHOLD: (process.env?.ANTIBAN_QUALITY_ALERT_THRESHOLD as AntiBanQuality) || 'YELLOW',
        AUTO_PAUSE_ON_RED_DAYS: Number.parseInt(process.env?.ANTIBAN_AUTO_PAUSE_ON_RED_DAYS) || 7,
      },
    };
  }
}
