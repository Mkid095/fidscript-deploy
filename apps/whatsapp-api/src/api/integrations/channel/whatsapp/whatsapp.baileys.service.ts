...
export class BaileysStartupService extends ChannelStartupService {
  ...
  /**
   * Send Interactive Buttons - New WhatsApp Business API format
   * Supports reply, url, and copy button types with richer formatting
   */
  public async interactiveButtonsMessage(data: SendInteractiveButtonsDto) {
    if (!data.action?.buttons || data.action.buttons.length === 0) {
      throw new BadRequestException('At least one button is required');
    }

    if (data.action.buttons.length > 3) {
      throw new BadRequestException('Maximum of 3 buttons allowed');
    }

    const buttons = data.action.buttons.map((btn: InteractiveButton) => {
      if (btn.type === 'reply') {
        return {
          name: 'quick_reply',
          buttonParamsJson: JSON.stringify({
            display_text: btn.reply?.title,
            id: btn.reply?.id || btn.reply?.title,
          }),
        };
        ...
        return {
          name: 'cta_url',
          buttonParamsJson: JSON.stringify({
            display_text: btn.url?.title,
            url: btn.url?.url,
            merchant_url: btn.url?.url,
          }),
        };
        ...
        return {
          name: 'cta_copy',
          buttonParamsJson: JSON.stringify({
            display_text: btn.copy?.title,
            copy_code: btn.copy?.copyCode,
          }),
        };
      }
      return null;
      ...
      viewOnceMessage: {
        message: {
          interactiveMessage: {
            header: data.header
              ? {
                  hasMediaAttachment: !!data.header.media,
                  ...(data.header.type === 'text' && { title: data.header.text }),
                  ...(data.header.type === 'image' &&
                    data.header.media && {
                      imageMessage: { url: data.header.media, mimetype: data.header.mimeType || 'image/jpeg' },
                    }),
                  ...(data.header.type === 'video' &&
                    data.header.media && {
                      videoMessage: { url: data.header.media, mimetype: data.header.mimeType || 'video/mp4' },
                    }),
                  ...(data.header.type === 'document' && {
                    documentMessage: {
                      url: data.header.media,
                      mimetype: data.header.mimeType || 'application/pdf',
            ...
            footer: data.footer ? { text: data.footer.text } : undefined,
            headerType: data.header
  ...
  /**
   * Send Product Message - Send a single product from catalog
   * Uses WhatsApp Shopping API
   */
  public async productMessage(data: SendProductDto) {
    const productMessage: proto.IMessage = {
      productMessage: {
        product: {
          productId: data.productId,
        },
        businessCatalogId: data.catalogId,
        ...(data.storefrontName && { storefrontName: data.storefrontName }),
        ...(data.sections && { productSections: data.sections }),
        header: data.header,
        body: data.body,
        footer: data.footer,
      },
    ...
    return await this.sendMessageWithTyping(data.number, productMessage, {
      delay: data?.delay,
      presence: 'composing',
      quoted: data?.quoted,
      mentionsEveryOne: data?.mentionsEveryOne,
  ...
  /**
   * Send Product Carousel - Send multiple products in a carousel format
   * Uses WhatsApp rich carousel message with products
   */
  public async productCarouselMessage(data: SendProductCarouselDto) {
    if (!data.productItems || data.productItems.length === 0) {
      throw new BadRequestException('At least one product item is required');
    }

    if (data.productItems.length > 10) {
      throw new BadRequestException('Maximum of 10 products allowed in carousel');
    }

    const carouselCards = data.productItems.map((item: ProductCarouselItem) => {
      const card: any = {
        product: {
      ...
      if (item.thumbnailUrl) {
        card.productImage = { url: item.thumbnailUrl };
      }
      ...
      if (item.title || item.description || item.price) {
        card.productInfo = {
          ...
          ...(item.price &&
            item.currency && {
              priceAmount: { value: parseFloat(item.price), offset: 100 },
        ...
        message: {
          interactiveMessage: {
            header: {
              ...
              ...(data.header.media && { imageMessage: { url: data.header.media, mimetype: data.header.mimeType || 'image/jpeg' } }),
            },
            body: { text: data.body.text },
            footer: data.footer ? { text: data.footer.text } : undefined,
            carouselMessage: {
  ...
  /**
   * Send Flow Message - Send a WhatsApp Flow
   * Used for complex forms and data collection
   */
      ...
      viewOnceMessage: {
        message: {
          interactiveMessage: {
            ...
            body: { text: data.data ? JSON.stringify(data.data) : '' },
            nativeFlowMessage: {
              buttons: [
                {
                  name: 'flow',
                  buttonParamsJson: JSON.stringify({
                    ...
                    ...(data.screen && { screen: data.screen.id }),
                    ...(data.action && { action: data.action.name }),
                    ...(data.final !== undefined && { final: data.final }),
                  }),
                },
              ],
              messageParamsJson: JSON.stringify({ from: 'api' }),
            },
          },
        },
      },
