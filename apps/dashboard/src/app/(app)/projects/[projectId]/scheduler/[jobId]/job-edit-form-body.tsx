    ...
    <form onSubmit={onSave} noValidate>
      <div className="space-y-5">
        ...
        <div>
          ...
          <Input value={formExpression} onChange={e => setFormExpression(e.target.value)}
            className="bg-[var(--surface-2)] border border-[var(--rail)] text-[var(--text)] w-full font-mono" />
          <div className="flex flex-wrap gap-1.5 mt-2">
            {CRON_PRESETS.map(p => (
              <button key={p.value} type="button" onClick={() => setFormExpression(p.value)}
          ...
          <div className="flex gap-4">
            {(['endpoint', 'function'] as const).map(t => (
              <label key={t} className="flex items-center gap-2 text-sm text-[var(--text-muted)] cursor-pointer">
                <input type="radio" name="editTargetType" value={t} checked={formTargetType === t}
                  onChange={() => setFormTargetType(t)} className="accent-[var(--accent)]" />
        ...
        {formTargetType === 'endpoint' ? (
          <div>
            ...
            <Input value={formEndpoint} onChange={e => setFormEndpoint(e.target.value)}
              className="bg-[var(--surface-2)] border border-[var(--rail)] text-[var(--text)] w-full" />
          </div>
        ) : (
          <div>
            <label className="block text-xs text-[var(--text-muted)] mb-1.5 font-medium">Function ID</label>
            <Input value={formFunctionId} onChange={e => setFormFunctionId(e.target.value)}
              className="bg-[var(--surface-2)] border border-[var(--rail)] text-[var(--text)] w-full" />
          </div>
        )}
        <div>
          <label className="block text-xs text-[var(--text-muted)] mb-1.5 font-medium">Payload (JSON)</label>
          <textarea value={formPayload} onChange={e => setFormPayload(e.target.value)} rows={3}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <Input type="number" min={0} max={10} value={formRetryAttempts}
          ...
          <div>
            ...
            <Input type="number" min={1} max={3600} value={formRetryDelay}
          ...
          <div>
            ...
            <Input type="number" min={1} max={3600} value={formTimeout}
