...
import { Button, Card, Badge } from '@fidscript/ui';
...
export function DomainCard({ domain, projectId, verifyingId, onVerify, onDelete, onShowInstructions }: DomainCardProps) {
  return (
    <Card className="border border-[var(--rail)] p-0 overflow-hidden" padding="none">
      <Link
      ...
      >
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex gap-1.5 ml-2 shrink-0">
            <Badge variant={dnsVariant(domain.dnsStatus)}>{domain.dnsStatus ?? 'PENDING'}</Badge>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Badge variant={sslVariant(domain.sslStatus)}>{domain.sslStatus ?? 'PENDING'}</Badge>
          </div>
        ...
        {(domain.type?.length ?? 0) > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {(domain.type ?? []).map((t: string) => {
              ...
              return <span key={t} className={`text-xs px-1.5 py-0.5 rounded-full ${color}`}>{label}</span>;
            })}
          </div>
        )}
        ...
        {(domain.dnsVerifiedAt || domain.routingVerifiedAt) && (
          <div className="text-xs text-[var(--text-muted)] mb-4 space-y-0.5">
            {domain.dnsVerifiedAt && <p>DNS verified: {new Date(domain.dnsVerifiedAt).toLocaleDateString()}</p>}
            {domain.routingVerifiedAt && <p>Routing verified: {new Date(domain.routingVerifiedAt).toLocaleDateString()}</p>}
          </div>
        )}
      </Link>
      ...
      <div className="border-t border-[var(--rail)] px-4 py-3 flex items-center gap-2 bg-[var(--surface-2)]">
        {domain.dnsMode === 'manual' && (
          <Button variant="ghost" size="sm" onClick={onShowInstructions}>DNS Instructions</Button>
        )}
        <div className="flex-1" />
        {(domain.dnsStatus === 'PENDING' || domain.dnsStatus === 'OWNERSHIP_PENDING' || domain.dnsStatus === 'VALIDATING') && (
          <Button variant="secondary" size="sm" loading={verifyingId === domain.id} onClick={onVerify}>Verify</Button>
        )}
        <Button variant="ghost" size="sm" className="text-[var(--danger)] hover:text-[var(--danger)]" onClick={onDelete}>Remove</Button>
      </div>
    </Card>
  );
}
