      ...
      <div className="overflow-auto">
        ...
        {Array.from({ length: maxLines }, (_, i) => {
          ...
          return (
            <div
              key={i}
              className={`px-2 py-0.5 ${changed ? 'bg-rose-500/10 text-rose-400' : 'text-[var(--text-muted)]'}`}
            >
              {l ?? ''}
            </div>
          );
      ...
      <div className="overflow-auto">
        ...
        {Array.from({ length: maxLines }, (_, i) => {
          ...
          return (
            <div
              key={i}
              className={`px-2 py-0.5 ${changed ? 'bg-emerald-500/10 text-emerald-400' : 'text-[var(--text-muted)]'}`}
            >
              {r ?? ''}
            </div>
          );
