/**
 * Mailbox CRUD — create / list / get / update / delete / suspend / activate
 * / password-reset. Owns the lifecycle; member management lives in
...
@Injectable()
export class MailboxCrudService {
  constructor(
  ...
  async createMailbox(projectId: string, dto: CreateMailboxDto, ownerUserId?: string) {
    ...
    // Stalwart account exists but the Prisma row is not yet committed — if
    // the DB writes fail, the Stalwart account would be orphaned. Wrap the
    // remaining DB writes in try/catch and best-effort delete the Stalwart
    // account on rollback (GAP-4 orphan fix).
    let mailbox;
    try {
      mailbox = await this.prisma.emailMailbox.create({
        data: {
      ...
      if (ownerUserId) {
        await this.prisma.emailMailboxMember.create({
          data: {
            ...
            userId: ownerUserId,
            role: 'OWNER',
            permissions: ['canReply', 'canDelete', 'canAssign', 'canExport', 'canManageMembers'] as object,
          },
        });
      }
    } catch (err) {
      // Best-effort rollback if Stalwart is reachable; if it isn't, the
      // original DB error is the actionable signal so we just rethrow.
      try { await this.stalwartAccount.deleteAccount(stalwartAccountId); } catch { /* swallow */ }
      throw err;
    }
  ...
  suspendMailbox(projectId: string, mailboxId: string) {
